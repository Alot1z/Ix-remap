...
> **MARK-READY ROUND ADDENDUM (2026-09-06 — the ticked Tier B GOs executed; live-verified same session):**
> 1. **#605 train release DONE:** `markPullRequestReadyForReview` executed → `draft:false, open`; **mandatory KB #6646 post-ready probe fired as predicted — code-owner request `josephismikhail` appeared** (unremovable once-drafted, GitHub bug #69208; reported, never removed; durable cure = the #608 CONTRIBUTING kit). Release note posted on #605 (comment `5555720298`, names final head `2f9604772c…`, byte-verbatim). CI at release: 28 success / 3 expected skips / 0 failures.
> 2. **Toolscan PR#1: ready then MERGED by the owner** (ready 2026-09-06T00:14Z, merged 00:18:39Z, merge commit `86b0da0c475a` = new main head; dispatch-check toolscan pin re-anchored to the merge in record commit `dcda446`).
> 3. **TUI prototype (item 9, local-only GO):** `tui-install-sketch.md` written, then built into a **working local prototype** `.fix605/tui-proto/ix-install-tui.mjs` (+ run-dir copy `run-2026-09-05-item9-train-bundle/tui-proto/`): banner via the real in-process renderer, menu rows from the engine's own `--dry-run` preview, byte-identical `--json`/`--dry-run`/`--help` pass-through, plan-only default. Never pushed, never opened.
> 4. **Logo ground truth (owner question "TUI CLI logo messy?"):** the renderer is NOT broken — `--bg brand` (shipped default, shown in the PR) renders clean `▀` blocks at 48/56; `--bg none` deliberately uses mixed `▄▀` edge shading (its shipped look since the `--bg none` supersession); truecolor SGR output pasted as text looks garbled by design. `#605 ships as reviewed` — no renderer change.

> **POST-RELEASE ROUND ADDENDUM (2026-09-06 — the four owner-selected follow-ups executed; #617/#618 opened draft-from-open per the DRAFT-ONLY PR SURFACE rule):**
> 1. **CONTRIBUTING durable-cure draft PR opened: ix-infrastructure/Ix#617** (`docs/contributing-draft-first` @ `4e028ee4f6`, based on current main `8c0e6b00eabc`): one-line Development-Workflow step-4 note teaching draft-first + the mark-ready code-owner request + the unremovable-request bug; Refs #608; gated snapshot→lease-push→verify→re-scan footer-free; `draft:true` at create, `requested_reviewers=[]` verified; body verbatim-verified.
> 2. **Installer UX re-derived + draft PR opened: ix-infrastructure/Ix#618** (`chore/installer-ux-labels-help` @ `b2664414c6`, based on main `8c0e6b00eabc`): U1 id-labeled lines, U3 real `--help` (arg parsing moved after the registry read so it lists real ids; unknown-option error gains the `--help` pointer), U4 dry-run footer naming `--json`; **the two CI install-smoke grep assertions moved in the same commit** (`would install [claude]:` shape) and each gained footer + `--help` pins. Refs #611. **Tested hermetically before push** (HOME-redirected fixtures + runner tree at `.fix605/tui-proto/`): `--help`/`--dry-run`/`--json`/refusal(exit 1, foreign SKILL.md untouched)/`--force` all hold; CI-assertion simulation at `/tmp/ix-harness-home` passes all new greps and the OLD grep correctly fails (proving the CI move is required and included). U2 (summary table) deliberately held for a follow-up PR.
> 3. **Issues cross-linked:** #611 ← comment `5555928919` (engine half = #618, table held for follow-up); #612 ← comment `5555928997` (interactive = pre-fill over the same engine, byte-identical parity, local prototype proving the mapping). Both footer-free, gated surface, author Alot1z.
> 4. **No new review activity on #605** (last review still github-advanced-security[bot]; last human comment still our reply + release note) — re-review watch continues via PLAYBOOK only.
