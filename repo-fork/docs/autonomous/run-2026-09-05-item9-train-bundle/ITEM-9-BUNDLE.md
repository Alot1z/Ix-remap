...
> **2026-09-05 draft-opens + close-out note (later same day):** with the owner's go, the prepared train kits opened as live drafts — **#609** (installer README-fix, `feat/installer-readme-fix` @ `b77c4f52`) and briefly **#610** (CONTRIBUTING draft-first note). **Same day the owner called #610 + #608 filler and had both closed** (superseded-by-process — no repo setting changes GitHub's ready-open code-owner behavior; D16). #605 also gained the **rendered banner preview** (PNGs at `output-samples/banner-48/80-truecolor.png`, head `c05c3a77`) + a picture in the body. **The train at mark-ready is now #605 + #609.** What remains gated on your go at this item: the rebase of #605 onto `e8ab1926` and mark-ready of the pair together. C3 stays held per its own kit.
...
**Prepared 2026-09-05 · this bundle is a local draft. Executing it (mark-ready, posting the release note) happens ONLY at item 9 with the owner's explicit go. Update: the two draft opens below already ran (2026-09-05, owner's go) — #609 + #610 are live drafts; the mark-ready sequence is what remains.**
...
1. **Pre-flight (read-only):** re-probe §0 rows — #605 still draft at `0869a137` with zero requested reviewers, toolscan HEAD `4c0b2d11` (owner brand pass 16:43Z; pin refreshed 2026-09-05 per KB #6644), wave MERGED — main head `e8ab1926` (re-probe it), so #605's branch sits behind main; run `dispatch-check.mjs` fast + `--live` (must be ALL GREEN).
