...
Posted as a comment on **#605** and on **#609** at the moment each is marked ready. One voice, no filler, nothing beyond what ships. Refreshed 2026-09-05 for the duo reality (#610/#608 closed as superseded-by-process) and the picture head.
...
>
> What this PR ships: the first-run logo banner — `scripts/render-logo.mjs` renders `assets/logo.png` at print time (zero dependencies: PNG decode → inflate → unfilter → supersample → flat 5-tone snap → half-block), truecolor with ANSI-256 and ASCII fallbacks, `--bg brand`/`--bg none`, honest `--json`, absent-safe fallback to the plain text heading, golden fixtures + hermetic pins, and rendered preview PNGs in `output-samples/` (embedded in this PR's description above). Rebased onto current main (`e8ab1926`); the rebase is verified conflict-free and the diff remains banner-only.
>
...
- The set named is the live duo (#605 + #609); toolscan work is named as context only (it ships via the repo's direct-commit convention, not a PR).
